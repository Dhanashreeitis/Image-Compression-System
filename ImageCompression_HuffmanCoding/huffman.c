...
int main(int argc, char** argv){
    ...
    if(image_file == NULL){
        ...
        offset = 2;
        fseek(image_file, offset, SEEK_SET);     //fseek used to move a file pointer associated to a given file to specific position
        fread(&imgsize, 4, 1, image_file);       //The ptr is the starting address of the memory block where data will be stored after reading from the file
   ...
   // Create a Histogram of the pixel intensity values present in the Image
    ...
    // finding the number of non-zero occurences
    int nodes = 0;
    for(i = 0; i < 256; i ++){
        if(histogram[i] != 0){
            nodes += 1;
    ...
    // calculating max length of huffman codeword
    i = 0;
    ...
    while(n < (nodes - 1)){
        ...
        pixFreq[node_next].left = &pixFreq[huffCodes[nodes - n - 2].arr_location];  //arr_location points to the location of child node in pixFreq array
